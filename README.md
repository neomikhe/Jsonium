...
- **Light and dark themes**, following your system by default, with a switch that overrides it. The
  theme and the language are the only things Jsonium ever writes to `localStorage` — never your
  document. Every text/background pair in both palettes passes WCAG AA
...
- **Español e inglés**, elegido según tu navegador y conmutable desde la cabecera
- **Tema claro y oscuro**, que sigue al sistema por defecto y se puede forzar desde la cabecera. El
  tema y el idioma son lo único que Jsonium escribe en `localStorage`: nunca tu documento. Todos los
  pares de texto y fondo de las dos paletas pasan WCAG AA
